...
exports.createUser = async function (req, res) {
  try {
    ...
    if (!validation.isValid(password))
      return res
        .status(400)
        .send({ status: false, message: "Pasworrd is required" });
    if (!validation.isValidPwd(password))
      return res
        ...
        .send({
          status: false,
          message:
            "Password length should be 8 to 15 digits and enter atleast one uppercase also one special character",
        });

    if (!address)
      return res.status(400).send({ status: false, msg: "address requried" });
    ...
    if (Object.keys(address) == 0)
      return res
        .status(400)
        .send({ status: false, message: "address field can't be empty" });

    if (!validation.isValid(address.shipping.street))
      return res
        .status(400)
        .send({ status: false, message: "street field is required" });
    if (!validation.isValid(address.shipping.city))
      return res
        .status(400)
        .send({ status: false, message: "city field is required" });
    if (!validation.isValid(address.shipping.pincode))
      return res
        .status(400)
        .send({ status: false, message: "pincode field is required" });
    if (!validation.isValidPincode(address.shipping.pincode))
      return res
        .status(400)
        .send({
          status: false,
          message: "PIN code should contain 6 digits only ",
        });
...
exports.loginUser = async function (req, res) {
  try {
    ...
    if (!findUser)
      return res
        .status(404)
        .send({ status: false, message: "the email id entered is wrong" });
    ...
    let token = jwt.sign({ userId: findUser._id }, "Products-Management", {
      expiresIn: "1d",
    });
  ...
  try {
    ...
    if (validation.isValidBody(data))
      return res
        ...
        .send({
          status: false,
          message: "Enter details to update your account",
        });
    if (fname) {
      if (!validation.isValid(fname))
        return res
    ...
    if (phone) {
      if (!validation.isValid(phone))
        return res
          .status(400)
          .send({ status: false, message: "phone is required" });
      if (!validation.isValidNum(phone))
        return res
          .status(400)
          .send({ status: false, message: "phone number is not valid" });
      let checkPhone = await userModel.findOne({ phone: phone });
    ...
    if (address || address == "") {
      if (!validation.isValid(address.trim()))
        return res
          .status(400)
          .send({ status: false, message: "Enter a valid type of address" });
      ...
      if (!validation.isValidPincode(address.shipping.pincode))
        return res
          ...
          .send({
            status: false,
            message: "Please enter a indian pincode correctly",
          });

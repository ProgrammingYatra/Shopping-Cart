  ...
  try {
    ...
    if (checkTitle)
      return res
        ...
        .send({
          status: false,
          message: "title already exist enter another title",
        });
    ...
    if (!validation.isValidPrice(price))
      return res
        ...
        .send({
          status: false,
          message: "price should be a number and not less than zero",
        });
    ...
    if (currencyId || currencyId == "") {
      ...
      if (currencyId != "INR")
        return res
          ...
          .send({
            status: false,
            message: "CurrencyId Should be INR only & in UpperCase",
          });
    }
    if (isFreeShipping) {
      if (isFreeShipping != true || false)
        return res
          ...
          .send({
            status: false,
            message: "isFreeShipping only contain a boolean value",
          });
    }
    ...
    for (let i = 0; i < availableSizes.length; i++) {
      if (!validation.isValidSize(availableSizes[i]))
        return res
          .status(400)
          .send({
            status: false,
            message: `Please mention valid Size between ${[
              "S",
              "XS",
              "M",
              "X",
    ...
    if (installments) {
      if (!validation.isValidNum(installments))
        return res
          ...
          .send({
            status: false,
            message: "please enter a valid installment number",
          });
    }
...
exports.getProduct = async function (req, res) {
  try {
    ...
    if (input.length > 0) {
      for (let i = 0; i < input.length; i++) {
        if (
          ...
          return res
            .status(400)
            .send({ status: false, message: "invalid filter condition" });
        }
      }
    }
    ...
    if (data?.size || typeof data.size == "string") {
      ...
      if (!validation.isValid(data.size))
        return res
          .status(400)
          .send({
            status: false,
    ...
    if (data?.name || typeof data.name == "string") {
      if (!validation.isValid(data.name))
        return res
          .status(400)
          .send({
            status: false,
    ...
    if (data?.priceGreaterThan || typeof data.priceGreaterThan == "string") {
      if (!validation.isValidString(data.priceGreaterThan))
        return res
          .status(400)
          .send({
            status: false,
      ...
      if (!validation.isValidNum(data.priceGreaterThan))
        return res
          .status(400)
    ...
    if (data?.priceLessThan || typeof data.priceLessThan == "string") {
      if (!validation.isValidString(data.priceLessThan))
        return res
          .status(400)
          .send({
            status: false,
      ...
      if (!validation.isValidNum(data.priceLessThan))
        return res
          .status(400)
          .send({ status: false, message: "Price of product should be valid" });
    ...
    if (data?.priceSort || typeof data.priceSort == "string") {
      if (data.priceSort != 1 && data.priceSort != -1)
        return res
          .status(400)
          .send({ status: false, message: "It should be only 1 or -1" });
      let getFilterData = await productModel
        .find(conditions)
...
exports.getProductsById = async (req, res) => {
  try {
    ...
    if (product.isDeleted == true)
      return res
        .status(404)
        .send({ status: false, message: "Product is already delted" });
  ...
  try {
    ...
    if (validation.isValidBody(data))
      return res
        ...
        .send({
          status: false,
          message: "please put required input to update",
        });

    if (!validation.isValidObjectId(productId))
      return res
        .status(400)
        .send({ status: false, message: "provide a valid productId" });
    ...
    if (findProduct.isDeleted == true)
      return res
        ...
        .send({
          status: false,
          message: "Product is already deleted you can't update it",
        });
    ...
    if (availableSizes) {
      ...
      for (let i = 0; i < availableSizes.length; i++) {
        if (!validation.isValidSize(availableSizes[i]))
          return res
            .status(400)
            .send({ status: false, message: "Please mention valid Size!" });
      }
    }
    if (installments) {
      if (!validation.isValidNum(installments))
    ...
    if (currencyFormat) {
      if (currencyFormat != "₹")
        return res
          .status(400)
          .send({ status: false, message: "it should be only ₹" });
    }
    if (currencyId || currencyId == "") {
      if (currencyId != "INR")
        return res
          .status(400)
          .send({ status: false, message: "it should be only INR" });
    }
    ...
    if (isDeleted || isDeleted == "") {
      if (isDeleted != "true" && isDeleted != "false")
        return res
          ...
          .send({
            status: false,
            message: "IsDeleted conly contain True or False",
          });
      if (isDeleted == "true") {
        data.isDeleted = true;
...
exports.deleteProduct = async function (req, res) {
  try {
    ...
    return res
      .status(200)
      .send({ status: true, message: "Product has been deleted successfully" });
  } catch (err) {
